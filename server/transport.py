...
Two connection shapes:
  ...
  * http  client-speaks-first SOAP. Read one request, route by URL path, answer, and close.
"""
...
# Seconds between heartbeats on an idle but logged-in GP connection (see gpcm.KEEPALIVE).
...
class HttpRouter:
    ...
    def route(self, head: str, body: str):
        ...
        # SakeFileServer is a plain GET, so it must be matched before the "/sake" test below, which
        # would otherwise swallow it and try to parse an empty GET body as an envelope.
        ...
        # Section 8's ATLAS SDK posts to /AtlasDataServices/GameConfig.asmx, not the
        # /CompetitionService/ path other GameSpy titles use. The SOAPAction test catches a third URL.
        if "/CompetitionService/" in head or "/AtlasDataServices/" in head or "gamespy.net/competition/" in soap_action:
        ...
        log.log(
            f"    [http] *** UNHANDLED PATH *** {request_line} SOAPAction={soap_action!r} "
...
class Server:
    ...
    def _handle(self, conn, addr, port, kind):
        ...
        try:
            ...
            # A client that drops a socket instead of shutting it down is normal here, not a fault.
            log.log(f"    [{port}] peer closed")
    ...
    def _handle_probe(self, conn, port):
        # Undeclared XLSP service: capture whatever the game sends so the protocol can be identified.
        # No data inside the timeout means the service is probably server-speaks-first.
        conn.settimeout(6.0)
        buf = b""
        try:
            while len(buf) < 16384:
    ...
    def _handle_gpcm(self, conn):
        ...
        while True:
            try:
                ...
                # GameSpy expects the server to drive this heartbeat, not the client.
                if gp.logged_in:
                    try:
    ...
    def _handle_http(self, conn, port):
        ...
        # CompetitionService captures the binary report to reports/, so keep it out of the text log.
        # Every other body is logged in full, however large, so a capture never silently drops one.
        ...
        if isinstance(routed, RawResponse):
            ...
            log.log(
                f"    [{port}] --> HTTP 200 ({len(routed.body)}B {routed.content_type})\n"

...
Section 8 uses LoginUniqueNick. The certificate signature is all-zero because the quick-patch neuters
the client's signature check; without that patch the client verifies it against a modulus baked into
the exe. The embedded profileid comes from the shared store, so it matches the identity GPCM issues.
"""
...
# A real 1024-bit RSA keypair (modulus, private exponent; public exponent 65537), each 128 bytes. The
# game parses these into bignums for the peer secure session, so a wrong-length or invalid keypair
# fails the certificate handshake before the patched-out signature check runs. One shared keypair is
# enough: peers encrypt to each other's public key, which round-trips against this private exponent.
PEER_MODULUS = (
    "DF1634EB0807695FEED8E3089F2D2F87BAD05C0545775CF9B505A2DA621A7D73"
    ...
    "830E3864FD684B8D588C40E187F5F17104D0FB3A43C91A8599ED504AD3169167"
)
PEER_PRIVATE = (
    "A78DDF4B13E9B52C779170DBFDEA0B43EF7D2550543F75969B6ED34520DFF28F"
    ...
    "50DBD524EF09EE2348EFC00B82ACE4840E53BACC4B2596026259062FE6AE2F19"
)
SERVER_DATA = "5A" * 128
SIGNATURE = "00" * 128

...
`S9GameInfoArcade` is the mode the game calls **Skirmish**:
...
Omit `?game=` and you get Conquest. **Assault and Skirmish additionally need the news file** (below) -
without it the server starts, resolves the mode, then exits with "This game mode is not available."

A wrong or misspelled `?game=` is never an error: the game silently falls back to the first mode the map
supports. Confirm which mode actually took effect from this server's own database, which stores the
`GameModeID` the server published:
...
A map URL is `<MapName>-<Variant>`, e.g. `TER01_Base-LargeA`. The engine splits the two itself, and the
variant decides which modes are legal on that map:
...
variant - guidance that says to look for a map file with "Assault" in its name is wrong, no such file
ships in any version or DLC. Swarm needs no `?game=` because `-SwarmA` supports exactly one mode.
...
map name is rejected at startup and dropped from the rotation. The mode is not repeated per entry - map
...
Assault and Skirmish are entitlement-gated behind an unlock class with no local criteria - originally
flipped server-side for everyone at the ten-million-kill milestone. This server opens it the way the game
intends, by serving a **news file**, so no binary patching is needed. The same file also carries the
welcome banner shown on the online menu.

Edit [`news/section8_news.txt`](news/section8_news.txt):
...
It is re-read on every request, so it can be edited against a running backend with no restart.

Three things about the format are unforgiving, and all three fail *silently*:

- the filter must be **`TGGameInfo`** (not `S9GameInfo`),
- class names must be **bare** (not `S9Game.S9GameInfoAssault`),
- a malformed line **aborts the rest of the section**, and in `[MOTD]` a comment merely mentioning the
  key is displayed instead of the real line.

The format, the failure modes and how to diagnose them are documented in
[`docs/game_modes_and_news.md`](docs/game_modes_and_news.md), along with why the modes are gated at all.
...
|----------|-------------|
| [`docs/game_modes_and_news.md`](docs/game_modes_and_news.md) | Game-mode selection, the Assault/Skirmish unlock gate, and the news file format |
